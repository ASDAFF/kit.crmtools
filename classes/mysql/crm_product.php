...
class CCrmProductTools
{
    ...
    {
        if (is_null(CCrmProductTools::$defaultCatalogId))
            self::$defaultCatalogId = self::EnsureDefaultExists();
    ...
    public static function Add($arFields)
    {
        ...
        if ($ID === null)
        {
            ...
            {
                $arElement['IBLOCK_ID'] = $arFields['CATALOG_ID'] = self::EnsureDefaultExists();
            }
...
//		$arInsert = $DB->PrepareInsert(CCrmProductTools::TABLE_NAME, $arFields);
//		$sQuery =
//			'INSERT INTO '.CCrmProductTools::TABLE_NAME.'('.$arInsert[0].') VALUES('.$arInsert[1].')';
    ...
    public static function Update($ID, $arFields)
    {
        ...
        else
        {
            if (isset($arFields['PRICE']) || isset($arFields['CURRENCY_ID']))
            {
                ...
                if ($price !== false && $currency !== false) CCrmProductTools::setPrice($ID, $price, $currency);
            }
        }

//		$sUpdate = trim($DB->PrepareUpdate(CCrmProductTools::TABLE_NAME, $arFields));
//		if (!empty($sUpdate))
//		{
//			$sQuery = 'UPDATE '.CCrmProductTools::TABLE_NAME.' SET '.$sUpdate.' WHERE ID = '.$ID;
//			$DB->Query($sQuery, false, 'File: '.__FILE__.'<br/>Line: '.__LINE__);
//
...
//		$sql = 'DELETE FROM '.CCrmProductTools::TABLE_NAME.' WHERE ID = '.$ID;
...
//
//            if($catalogID > 0 && !self::Exists($catalogID))
//            {
//                $catalogID = 0;
//            }
//            if($catalogID <= 0)
//            {
//                $catalogID = self::EnsureDefaultExists();
//            }

            $arFilterRewrited['IBLOCK_ID'] = $arFilter['CATALOG_ID'];//$catalogID;
//        }
        // </editor-fold>
    ...
    {
        return true;
        $dbRes = CCrmProductTools::GetList(array(), array('ID'=> $ID), array('ID'));
        return $dbRes->Fetch() ? true : false;
    }

    public static function EnsureDefaultCatalogScope($productID)
    {
        $dbRes = CCrmProductTools::GetList(array(), array('ID'=> $productID), array('ID', 'CATALOG_ID'));
        $fields = $dbRes->Fetch();
        return is_array($fields) && isset($fields['CATALOG_ID']);
    }
...
//        $arFilter['CATALOG_ID'] = 29;

        $dbRes = CCrmProductTools::GetList(array(), $arFilter, array('*'), array('nTopCount' => 1));
    ...
    {
        $dbRes = CCrmProductTools::GetList(array(), array('NAME' => strval($name)), array('*'), array('nTopCount' => 1));
    ...
    public static function GetByOriginID($originID, $catalogID = 0)
    {
        ...
        $dbRes = CCrmProductTools::GetList(array(), array('CATALOG_ID' => $catalogID, 'ORIGIN_ID' => $originID),
...
//			'SORT' => array('FIELD' => 'E.SORT', 'TYPE' => 'int', 'FROM' => 'INNER JOIN b_iblock_element E ON P.ID = E.ID')
    ...
    {
        return CCrmProductTools::Delete($ID);
    }
    // <-- Event handlers
